...
class OurTrainer(Trainer):
    ...
    def zo_perturb_parameters(self, random_seed=None, scaling_factor=1):
        ...
        # 为了避免存储引导向量，我们直接在这里计算需要的扰动
        if hasattr(self, 'zo_random_seed_mgfwa') and hasattr(self, 'split_ratio') and hasattr(self, 'fw_seeds') and hasattr(self, 'fw_fitness'):
            # 使用分组比例计算参与计算的烟花数量
            fitness_tensor = torch.tensor(self.fw_fitness, device=next(iter(param.device for _, param in self.named_parameters_to_optim)))
            sort_idx = torch.argsort(fitness_tensor)
            
            num_top = int(len(self.fw_fitness) * self.split_ratio)
            ...
            for name, param in self.named_parameters_to_optim:
                ...
                # 实时计算引导扰动
                guided_z = torch.zeros(shape, device=device, dtype=dtype)
                
                # 计算 top group 的平均扰动
                for idx in top_idx:
                    temp_seed = self.fw_seeds[idx]
                    torch.manual_seed(temp_seed)
                    z = torch.normal(mean=0, std=1, size=shape, device=device, dtype=dtype)
                    guided_z.add_(z / num_top)
                
                # 减去 bottom group 的平均扰动
                for idx in bottom_idx:
                    temp_seed = self.fw_seeds[idx]
                    torch.manual_seed(temp_seed)
                    z = torch.normal(mean=0, std=1, size=shape, device=device, dtype=dtype)
                    guided_z.sub_(z / (len(self.fw_fitness) - num_top))
                
                # 应用扰动
                param.data = param.data + scaling_factor * guided_z * self.args.zo_eps
        else:
            # 回退到原始的 MeZO 扰动方式
            for name, param in self.named_parameters_to_optim:
        ...
        if hasattr(self, 'zo_random_seed_mgfwa') and hasattr(self, 'split_ratio') and hasattr(self, 'fw_seeds') and hasattr(self, 'fw_fitness'):
            ...
            for name, param in self.named_parameters_to_optim:
                ...
                if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                    param.data = param.data - self._get_learning_rate() * (self.projected_grad * guided_z + args.weight_decay * param.data)
                else:
                    param.data = param.data - self._get_learning_rate() * (self.projected_grad * guided_z)
        else:
            # 回退到原始的 MeZO 更新方式
            for name, param in self.named_parameters_to_optim:
                z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
    ...
    def _compute_mgfwa_fitness(self, model, inputs):
        # 超参数
        base_fw_size = 2  # 基础烟花数
        self.split_ratio = 0.5  # 分组比例
        ...
        for fw_idx in range(base_fw_size):
            ...
            self.fw_seeds.append(random_seed)
            
            # 应用扰动
            torch.manual_seed(random_seed)
            for name, param in self.named_parameters_to_optim:
                z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                param.data = param.data + z * self.args.zo_eps
            
            # 计算适应度
            fitness = self.zo_forward(model, inputs).item()
            self.fw_fitness.append(fitness)
            
            # 恢复参数：直接使用相反的扰动而不是存储原始参数
            torch.manual_seed(random_seed)  # 使用相同的种子以获得相同的扰动
            for name, param in self.named_parameters_to_optim:
                z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                param.data = param.data - z * self.args.zo_eps  # 减去之前加入的扰动
    ...
    def zo_step(self, model, inputs):
        ...
        self._compute_mgfwa_fitness(model, inputs)
