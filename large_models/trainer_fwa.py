...
class OurTrainer(Trainer):
    ...
    def _compute_guided_vector_with_history(self, model, inputs):
        """
        使用逐步累积均值计算指导向量，减少存储开销。
        """
        # 超参数
        base_fw_size = 2  # 基础烟花数
        split_ratio = 0.5  # 分组比例
        ...
        # 历史种子与当前种子合并
        all_seeds = curr_perturbation_seeds
        all_fitness = curr_fitness

        # 计算指导向量时避免全存储：动态累积均值
        fitness_tensor = torch.tensor(all_fitness, device=next(iter(param.device for _, param in self.named_parameters_to_optim)))
        sort_idx = torch.argsort(fitness_tensor)

        num_top = int(len(all_fitness) * split_ratio)
        top_idx = sort_idx[:num_top]
        bottom_idx = sort_idx[num_top:]
        ...
        self.guided_vector = {}
        for name, param in self.named_parameters_to_optim:
            self.guided_vector[name] = torch.zeros_like(param.data, device=param.data.device, dtype=param.data.dtype)
        
        ########
        
        top_seed_idx = top_idx[0]
        random_seed = all_seeds[top_seed_idx]
        torch.manual_seed(random_seed)
        for name, param in self.named_parameters_to_optim:
            z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
            self.guided_vector[name].add_(z) 
        #########
        
        '''
        # 计算 Top 和 Bottom 累积均值
        for idx in top_idx:
            random_seed = all_seeds[idx]
            torch.manual_seed(random_seed)
            for name, param in self.named_parameters_to_optim:
                z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                self.guided_vector[name].add_(z / num_top)  # 动态更新 top_mean
        ...
        for idx in bottom_idx:
            ...
            for name, param in self.named_parameters_to_optim:
                z = torch.normal(mean=0, std=1, size=param.data.size(), device=param.data.device, dtype=param.data.dtype)
                self.guided_vector[name].sub_(z / (len(all_fitness) - num_top))  # 动态更新 bottom_mean 并计算差值
        '''    
